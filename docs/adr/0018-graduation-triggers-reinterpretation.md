...
| 4 | Distribution verified on all 5 RIDs | Verified during `0.1.0` release (#19): `npm install -g mssql-mcp` + `dotnet tool install -g mssql-mcp` on linux-x64, linux-arm64, osx-x64, osx-arm64, win-x64. Each must launch and print `0.1.0`. | Maintainer (HITL) | #19 |
