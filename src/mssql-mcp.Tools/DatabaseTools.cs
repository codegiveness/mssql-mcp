...
public sealed class DatabaseTools
{
    ...
    public async Task<string> ListDatabases(CancellationToken ct)
    {
        _logger.LogInformation("[tool] list_databases invoked");
        List<Dictionary<string, object?>> rows = await _executor.ExecuteQueryAsync(ListDatabasesSql, ct).ConfigureAwait(false);
